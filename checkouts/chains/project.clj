(defproject chains "0.0.1-SNAPSHOT"
  ...
  :plugins [[info.sunng/lein-bootclasspath-deps "0.2.0"]
            ...
            [lein-codox "0.9.5"]]
  :aliases {"migrate"  ["run" "-m" "chains.db/migrate"]
            "rollback" ["run" "-m" "chains.db/rollback"]
            "autotest" ["with-profile" "test" "midje" ":autotest"]}
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "chains.server/run-dev"]}
             ...
             :test {:env {:kafka-server "localhost:9091"
                    ...
                    :dependencies [[io.pedestal/pedestal.service-tools "0.5.0" :exclusions [org.clojure/tools.reader]]]}
             :uberjar {:aot :all}}
  :aot [chains.config chains.kafka]
  :main ^{:skip-aot true} chains.server)
